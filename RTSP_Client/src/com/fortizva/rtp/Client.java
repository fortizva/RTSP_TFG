...
public class Client {
	...
	public Client() {
		...
		/*videoTimer = new Timer(CommonPacketValues.PLAYBACK_FRAME_PERIOD, new videoTimerListener());
		videoTimer.setInitialDelay(0);
		videoTimer.setCoalesce(true);
		
		audioTimer = new Timer(CommonPacketValues.PLAYBACK_AUDIO_FRAME_PERIOD, new audioTimerListener());
	...
	class videoTimerListener implements Runnable {
		public void run() {
			while(running) {
				...
				if(!running) break; // Exit if the running flag is false
				try {
		...
		try {
			...
			*/
			if (request_type.equals("SETUP")) {
				RTSPBufferedWriter.write("Transport: RTP/UDP; client_port= " + CommonValues.RTP_RCV_PORT + CommonValues.CRLF);
				if (verbose)
					System.out.println("C: " + "Transport: RTP/UDP; client_port= " + CommonValues.RTP_RCV_PORT + CommonValues.CRLF);
			} else {
