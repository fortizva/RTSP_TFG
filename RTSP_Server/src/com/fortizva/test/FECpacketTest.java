...
class FECpacketTest {
    ...
    public void testFECpacketConstructorAndByteArray() {
        ...
        // Check PT (payload type, should match RTP packets XOR) (We can ignore the M,
        // as it is not used in this implementation)
        // XORing ptype 3 times should result in ptype itself, since ptype is the same
        // for all packets. But improves readability.
        assertEquals(ptype ^ ptype ^ ptype, fecBytes[1] & 0x7F); // Masking the M for payload type

        // Check SN base (2 bytes)
        // Shift right to discard lower bits and mask with 0xFF to discard the upper
        // bits and any sign extension
        ...
        // Check protection mask (next 2 bytes, should be 0b11100000_00000000 for 3
        // packets)
    ...
    void testFECRecoveryOfAllPackets() {
        ...
        // Try to recover each packet by removing it from the array and using the other
        // two
